...
class Model(nn.Module):
    def __init__(self, num_class=60, num_point=25, num_person=2, graph=None, graph_args=dict(), in_channels=3,out_channels=3):
        ...
        self.bn_x1 = nn.BatchNorm1d(num_person * in_channels * num_point)
        self.bn_x2 = nn.BatchNorm1d(num_person * in_channels * num_point)

        self.cnn1 = nn.Conv1d(1000, 512, 1)
        self.cnn2 = nn.Conv1d(512, 256, 1)
        self.cnn3 = nn.Conv1d(256, 45, 1)

        self.lstm = nn.LSTM(input_size=90, hidden_size=128, num_layers=2, batch_first=True)
    ...
    def forward(self, x1,x2):
        N, C, T, V, M = x1.size()
        edge_index_device = self.edge_index.to(x1.device)

        # Flatten the input data to align with ChebConv requirements
        x1 = x1.permute(0,4,2,3,1).contiguous().view(N * M * T , V , C)

        # ChebConv layer
        x1 = self.cheb_conv(x1, edge_index_device)
        x1 = self.dropout1(x1)

        # LSTM layer
        x1 = x1.view(-1, V * C)
        x1 = self.bn_x1(x1).view(N * M, T, V * C)
...
# AGCN + LSTM without RGB componentt
...
class Model(nn.Module):
    def __init__(self, num_class=60, num_point=25, num_person=2, graph=None, graph_args=dict(), in_channels=3):
        ...
        self.lstm = nn.LSTM(256 * num_point, 256, batch_first=True)
    ...
    def forward(self, x):
        ...
        # N*M,C,T,V
        x = x.permute(0, 1, 3, 2)
        x = x.contiguous().view(N, M, -1, T).mean(1)
